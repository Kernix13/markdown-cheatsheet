...
They are all inline except for the `img` tags. I assume some people put links around the icons so that they are inlib=ne. That's fine I guess, but I think a span tag with `align="left"` or `center` would be best, although you could do `right` to have them stand out or as a visual change to the normal left alignment.
